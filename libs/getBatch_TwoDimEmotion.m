function [imBatch, labelBatch] = getBatch_TwoDimEmotion(imdb, batch, mode, opts)
% -------------------------------------------------------------------------

imBatch = zeros(imdb.meta.imagesize(1),imdb.meta.imagesize(2),imdb.meta.imagesize(3),length(batch),'single');
labelBatch = zeros(1,1,2,length(batch),'single');
if ~isempty(batch)
    for i = 1:length(batch)
        ...
        label = label(1:2);
        labelBatch(:,:,:,i) = single(reshape(label,[1 1 2]));        
    end
end
