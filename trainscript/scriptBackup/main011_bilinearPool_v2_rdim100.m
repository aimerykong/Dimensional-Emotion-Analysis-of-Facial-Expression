...
lName = 'output';
dimInput = dimOutput*dimOutput;
dimOutput = 3;
netbasemodel.addLayer(lName , ...
    dagnn.Conv('size', [1 1 dimInput 3]), ...
    sName, lName, {'output_conv_f', 'output_conv_b'}) ;
ind = netbasemodel.getParamIndex('output_conv_f');
weights = randn(1, 1, dimInput, dimOutput, 'single')*sqrt(2/dimOutput);
netbasemodel.params(ind).value = weights;
ind = netbasemodel.getParamIndex('output_conv_b');
weights = zeros(1, dimOutput, 'single'); 
netbasemodel.params(ind).value = weights;
sName = lName;
...
netbasemodel.params(netbasemodel.getParamIndex('output_conv_f')).learningRate = 10;
netbasemodel.params(netbasemodel.getParamIndex('output_conv_b')).learningRate = 20;
...
opts.expDir = fullfile('./exp', 'main011_bilinearPool_v2_rdim100');
